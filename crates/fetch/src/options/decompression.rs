...
impl ResponseDecompressionOptions {
    ...
    pub(crate) fn limits(&self) -> Option<compressors::DecompressorLimits> {
        ...
        let limits = match self.output_limit {
            ...
            OutputLimit::Bounded(bytes) => limits.max_output_len(
                NonZeroU64::new(bytes).expect("ResponseDecompressionOptions::max_output_len rejects zero"),
            ),
            OutputLimit::Unbounded => limits.unbounded_output_len(),
        };

        Some(match self.max_streams {
            Some(streams) => limits.max_streams(
                NonZeroU64::new(streams).expect("ResponseDecompressionOptions::max_streams rejects zero"),
            ),
            None => limits,
        })
    }
...
mod tests {
    ...
    fn explicit_unbounded_output_is_distinct_from_inheritance() {
        ...
        assert_eq!(options.limits(), Some(compressors::DecompressorLimits::new().unbounded_output_len()));
    }
